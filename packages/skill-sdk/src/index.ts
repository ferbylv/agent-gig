...
export function createSkillClient(opts: SkillClientOptions) {
  ...
  return {
    ...
    createOrder: (input: { providerAgentId: string; feeCap: number; taskSummary: string }) => api(baseUrl, "/v0/orders", { method: "POST", body: JSON.stringify({ ...input, hirerUserId: userId, hirerAgentId }) }, userId, hirerAgentId),
